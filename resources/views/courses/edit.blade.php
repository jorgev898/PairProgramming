...
<head>
    ...
    <title>{{ __('Edit Course') }} — PairSync</title>
            ...
            <div class="row">
                <div class="col" style="flex: 0 0 100px;">
                    <div class="form-group">
                        ...
                        <input type="text" name="icon" class="form-control" value="{{ old('icon', $course->icon) }}" required>
                    </div>
                </div>
                <div class="col">
                    <div class="form-group">
            ...
            <div class="row">
                ...
                <div class="col">
                    <div class="form-group">
                        ...
                        <input type="color" name="color" class="form-control" value="{{ old('color', $course->color) }}" required style="padding: 4px; height: 46px;">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div id="lessons-container">
                @foreach($course->lessons as $index => $lesson)
                    <div class="lesson-card" id="lesson-{{ $index }}">
                        <input type="hidden" name="lessons[{{ $index }}][id]" value="{{ $lesson->id }}">
                        <div class="lesson-header">
                            <span class="lesson-title">Lesson #{{ $index + 1 }}</span>
                            <div>
                                <a href="{{ route('lessons.edit', $lesson->id) }}" style="display:inline-flex; align-items:center; gap:6px; font-size:0.8rem; padding:4px 10px; border-radius:4px; text-decoration:none; color:var(--primary); background:rgba(99, 102, 241, 0.1); border:1px solid rgba(99, 102, 241, 0.2); margin-right:8px; font-weight:600;">✨ {{ __('Edit Content') }}</a>
                                <button type="button" class="btn-remove-lesson" onclick="removeLesson({{ $index }})">{{ __('Remove') }}</button>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col" style="flex: 0 0 80px;">
                                <div class="form-group">
                                    <label>Icon</label>
                                    <input type="text" name="lessons[{{ $index }}][icon]" class="form-control" value="{{ $lesson->icon }}">
                                </div>
                            </div>
                            <div class="col">
                                <div class="form-group">
                                    <label>Title</label>
                                    <input type="text" name="lessons[{{ $index }}][title]" class="form-control" value="{{ $lesson->title }}" required>
                                </div>
                            </div>
                            <div class="col" style="flex: 0 0 120px;">
                                <div class="form-group">
                                    <label>Duration</label>
                                    <input type="text" name="lessons[{{ $index }}][duration]" class="form-control" value="{{ $lesson->duration }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <label class="checkbox-label">
                            <input type="hidden" name="lessons[{{ $index }}][available]" value="0">
                            <input type="checkbox" name="lessons[{{ $index }}][available]" value="1" {{ $lesson->available ? 'checked' : '' }}>
